...
namespace iSpyApplication.Controls
{
    class VideoPlayback:PictureBox
    {
        ...
        private void SetSize()
        {
            ...
            {
                fw = _lastFrame.Width;
                fh = _lastFrame.Height;
                if (Height > 0 && Width > 0)
                {
                    double arw = Convert.ToDouble(Width) / Convert.ToDouble(fw);
                    double arh = Convert.ToDouble(Height) / Convert.ToDouble(fh);
                    if (arh <= arw)
                    {
        ...
        protected override void  OnMouseMove(MouseEventArgs e)
        {
            ...
            if (_tbVolume.Nav)
            {
                ...
                if (VolumeChanged != null)
                    VolumeChanged(this, _tbVolume.Val);
            }
